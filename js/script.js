...
var xbmc_youtube = {
    ...
    finders:{
        ...
        thumb:function (element, hook, hook_type) {
            return $(element).closest('[href]').attr('href').split('v=')[1].split("&")[0];
        },
        playlist:function (element, hook, hook_type) {
            return $(element).closest('[href]').attr('href').split('list=')[1].split("&")[0];
        }
    ...
    inserters:{
        after_player:function (element, hook, hook_type, item_id) {
            ...
            tpl = tpl.replace("##id##", item_id);
            tpl = tpl.replace("##type##", "video");
